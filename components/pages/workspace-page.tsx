...
export function WorkspacePageClient({
  ...
  const {
    fetchNextPage: fetchNextUnsplashPage,
    hasNextPage: hasNextUnsplashPage,
    isFetchingNextPage: isFetchingNextUnsplashPage,
  } = unsplashResults;
  ...
  useEffect(() => {
    if (pageQuery.data?.page.workspaceId && pageQuery.data.page.workspaceId !== workspaceId) {
      router.replace(`/${pageQuery.data.page.workspaceId}/${pageQuery.data.page.id}`);
    }
  }, [pageQuery.data?.page.id, pageQuery.data?.page.workspaceId, router, workspaceId]);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (entry.isIntersecting && hasNextUnsplashPage && !isFetchingNextUnsplashPage) {
          fetchNextUnsplashPage();
        }
      },
      { threshold: 0.4 },
    );
  ...
  return (
    <div className="flex h-screen bg-[#fbfbfa] dark:bg-[#1b1a19]">
      {isOpen && <Sidebar workspaceId={workspaceId} activePageId={pageId} />}
